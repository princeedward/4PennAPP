...
void turretDrive(){
	...
	char TU;
	char TH;
	if (receive_buffer[2]==1 && receive_buffer[1]==0 )
	{
		left = 75;
		right = 75;
	...
	{
		if (receive_buffer[2]==0 && receive_buffer[1]==1 )
		{
			left = -75;
			right = -75;
		}else
	...
	if (receive_buffer[0]==1 && receive_buffer[3]==0 )
	{
		left += 25;
		right+=-25;
	}else
	{
		if (receive_buffer[0]==0 && receive_buffer[3]==1)
		{
			left +=-25;
			right+= 25;
		}
	}
	if (receive_buffer[5]==1 && receive_buffer[7]==0)
	{
		TH =  -1;
	}else
	{
		if (receive_buffer[5]==0 && receive_buffer[7]==1)
		{
			TH =  1;
		}else
		{
	...
	if (receive_buffer[4]==1 && receive_buffer[6]==0)
	{
		TU =  1;
	}else
	{
		if (receive_buffer[4]==0 && receive_buffer[6]==1)
		{
			TU =  -1;
		}else
		{
...
void set_move( int left, int  right , char turretLR, char turretUD){
	...
	if	( right < 0 )	{ OCR1B = (int)((-right)*(float)(MaxSpeed/100)); }
	else if ( right > 0  )	{ OCR1B  = (int)(right*(float)(MaxSpeed/100)); }
	else {OCR1B = 0 ;}

	if 	( left < 0 ) { OCR1C = (int)((-left)*(float)(MaxSpeed/100)); }
	else if ( left >  0 ) { OCR1C  = (int)(left*(float)(MaxSpeed/100)); }
	else {OCR1C = 0 ;}

	if 	(left > 0)	{   set(PORTC,6); }
	else if (left < 0)	{ clear(PORTC, 6); }	

	if 	(right > 0)	{   set(PORTC,7); }
	else if (right < 0)	{ clear(PORTC, 7); } 
...
void  setup_timer_1(){
	OCR1A = MaxSpeed;		OCR1B=0; 		OCR1C=0;
